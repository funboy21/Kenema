...
if($pass1 != $pass2){
    ...
    $check_sql="select * from users where email='$email' OR phone_number='$mobile' ";
    ...
    if (count($errors) == 0){
            //submit data
    $sql="insert into users (firstname,lastname,email,phone_number,password) values ('$first_name','$last_name','$email','$mobile','$pass1')  ";
...
if(isset($_POST['plogin'])){
  ...
  if (empty($phar_password)) {
    ...
    if (mysqli_num_rows($query) == 1) {
  	  ...
  	  header('location: Pharma/pharmaindex.php');
  	}else {
  		array_push($errors, "Wrong username/password combination");
  	}
    }
